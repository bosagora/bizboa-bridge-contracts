...
// @ts-ignore
import { MetaTokenSwap, TestERC20 } from "../../typechain";
import { BOAToken, ContractUtils } from "../ContractUtils";

describe("Test of MetaTokenSwap Contract", () => {
    let bizToken: TestERC20;
    let metaSwap: MetaTokenSwap;
    ...
    before(async () => {
        ...
        const swap = await ethers.getContractFactory("MetaTokenSwap");
    ...
    it("Check the biz token status", async () => {
    ...
    it("Send liquidity", async () => {
        ...
        await expect(metaSwap.connect(ownerSigner).increaseLiquidity(liquidityAmount)).to.emit(
            metaSwap,
            "IncreasedLiquidity"
        );
